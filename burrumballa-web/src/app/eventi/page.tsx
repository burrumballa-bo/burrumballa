...
function eventBottomLine(event: EventItem): string | null {
  ...
  return event.event_time
    ? `${event.event_time}${event.note ? ` · ${event.note}` : ""}`
    : event.note;
}
  ...
  const content = (
    <div className="border-bb-ink grid grid-cols-1 overflow-hidden rounded-[5px] border-[3px] md:grid-cols-[1fr_1.1fr]">
      ...
      >
        ...
        {event.subtitle && (
          <div className="font-marker mt-1.5 text-lg text-[#f6a323]">
            {event.subtitle}
          </div>
        )}
        {event.body && (
...
function EventCard({
  ...
  const content = (
    <div className="border-bb-ink bg-bb-surface grid h-full grid-cols-1 overflow-hidden rounded-[5px] border-[3px] sm:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="p-5.5">
        ...
        {event.body && (
          <p className="text-[13px] leading-relaxed">{event.body}</p>
        )}
        {bottomLine && (
          <div className="mt-2.5 text-xs font-bold">{bottomLine}</div>
        )}
      </div>
    </div>
  );
  ...
  return event.cta_url ? (
    <Link href={event.cta_url} className={cn("block h-full", className)}>
      {content}
    </Link>
  ) : (
...
export default async function EventiPage() {
  ...
  return (
    <SiteShell active="eventi" footer={footer}>
      ...
      {rest.length > 0 &&
        (() => {
          const mdFix = rowNeedsCentering(rest.length, 2);
          return (
            <div
              className={cn(
                "mx-auto grid max-w-[1200px] grid-cols-1 gap-5 px-6 pt-10",
                mdFix
            ...
            >
              {rest.map((event) => (
                <EventCard
                  ...
                  className={
                    mdFix
                      ? "md:shrink-0 md:basis-[calc(50%-0.625rem)]"
                      : undefined
                  }
                />
              ))}
