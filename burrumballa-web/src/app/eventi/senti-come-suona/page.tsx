...
export default async function SentiComeSuonaPage() {
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col">
      ...
      <section className="relative bg-[#050505]">
        <div className="relative h-72 overflow-hidden sm:h-80">
          ...
          <div className="absolute bottom-4 left-5">
            <h1 className="font-[family-name:var(--font-anton)] text-3xl leading-[1.05] text-white uppercase sm:text-4xl">
              {eventInfo.titolo}
            </h1>
          </div>
        </div>
        <div className="flex items-baseline justify-between gap-4 bg-[#111] px-5 py-4">
          <div className="font-[family-name:var(--font-anton)] text-4xl leading-none text-[#f5d90a] sm:text-5xl">
      ...
      {/* Descrizione + info */}
      <section className="border-t-2 border-[#7c1fd6] bg-black px-5 py-5">
        {eventInfo.descrizione && (
      ...
      {/* Giuria / Host / DJ */}
      <section className="bg-[#050505] px-5 py-6">
        <p className="font-[family-name:var(--font-anton)] mb-1 text-sm tracking-[0.08em] text-white uppercase">
          Giuria
        </p>
        ...
        <div className="mb-6 grid grid-cols-2 gap-2.5">
          {GIURIA.map((persona) => (
            <div key={persona.nome} className="flex flex-col gap-1.5">
              ...
              <div>
                <p className="font-[family-name:var(--font-anton)] text-[15px] text-white">
                  {persona.nome}
                </p>
                <p className="text-[8px] font-bold tracking-[0.1em] text-[#f5d90a] uppercase">
                  {persona.ruolo}
                </p>
              </div>
            </div>
        ...
        <p className="font-[family-name:var(--font-anton)] mb-2.5 text-xs tracking-[0.08em] text-white uppercase">
          Host · DJ
        </p>
        <div className="grid grid-cols-3 gap-2">
          {HOST_DJ.map((persona) => (
            <div key={persona.nome} className="flex flex-col gap-1">
              <PlaceholderPhoto label="FOTO" />
              <div>
                <p className="font-[family-name:var(--font-anton)] text-[11px] text-white">
                  {persona.nome}
                </p>
                <p
      ...
      <section className="border-t-2 border-[#f5d90a] bg-[#111] px-5 pt-7 pb-2">
        <h2 className="font-[family-name:var(--font-anton)] text-[28px] tracking-[0.03em] text-white uppercase sm:text-[30px]">
          Iscrizione
        </h2>
      ...
      <div id="iscrizione" className="scroll-mt-8 bg-[#050505] px-5 py-6">
        <RegistrationForm />
      </div>
    </main>
  )
